...
public class Fly : MonoBehaviour
{
    ...
    public List<Transform> GetNeighbours()
    {
        ...
        {
            if (
                flockMember.GetComponent<Fly>().State != FlyState.Dead
    ...
    public List<Transform> GetBubbles()
    {
        List<Transform> bubbles = new List<Transform>();
        GameObject bubblesList = GameObject.Find("Bubbles");
        if (bubblesList != null)
        {
            foreach (Transform bubble in bubblesList.transform)
            {
    ...
    void FixedUpdate()
    {
        ...
        List<Transform> bubbles = GetBubbles();
        ...
        {
            if (
                _frogRb.velocity.magnitude >= FrogAlertSpeed
                    && (
                        (transform.position - _frog.transform.position).magnitude
                        < FrogMovingFleeRange
                    )
                || _frogRb.velocity.magnitude < FrogAlertSpeed
                    && (
        ...
        if (bubbles != null)
        {
            ...
            {
                float distance = (transform.position - bubble.transform.position).magnitude;

                if (distance <= BubbleFleeRange)
                {
        ...
        switch (State)
        {
            case FlyState.Flocking:
                ...
                Vector2 desiredSep =
                    _settings.SeparationWeight
                    * Steering.GetSeparation(transform.position, neighbours, _settings.MaxSpeed);
                Vector2 desiredCoh =
                    _settings.CohesionWeight
                    * Steering.GetCohesion(transform.position, neighbours, _settings.MaxSpeed);
                Vector2 desiredAli =
                    _settings.AlignmentWeight
                    * Steering.GetAlignment(neighbours, _settings.MaxSpeed);
                Vector2 desiredAnch =
                ...
                Debug.DrawLine(
                    transform.position,
                    (Vector2)transform.position + desiredSep,
                    Color.red
                );
                Debug.DrawLine(
                    transform.position,
                    (Vector2)transform.position + desiredCoh,
                    Color.green
                );
                Debug.DrawLine(
                    transform.position,
                    (Vector2)transform.position + desiredAli,
                    Color.blue
                );
                Debug.DrawLine(
                    transform.position,
                    (Vector2)transform.position + desiredAnch,
                    Color.yellow
                );

                desiredVel =
                    (desiredSep + desiredCoh + desiredAli + desiredAnch).normalized
                    * _settings.MaxSpeed;
                break;
        ...
        Vector2 steering = Steering.DesiredVelToForce(
            desiredVel,
            _rb,
            _settings.AccelTime,
            _settings.MaxAccel
    ...
    public void HandleEvent(FlyEvent e)
    {
        ...
        switch (State)
        {
            case FlyState.Flocking:
                ...
                else if (e == FlyEvent.ScaredByBubble)
                {
                    SetState(FlyState.Fleeing);
                }
                break;
